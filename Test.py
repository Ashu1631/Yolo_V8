...
for p in pages:
    ...
    # Inject per-button GREEN override when selected
    if is_selected:
        st.sidebar.markdown(f"""
            <style>
            /* Target the specific button by its key */
            div[data-testid="stButton"] button[kind="secondary"]:has(+ * [data-key="nav_{p}"]),
            section[data-testid="stSidebar"] div[data-testid="stButton"][data-key="nav_{p}"] button {{
                background: linear-gradient(135deg, #155724, #28a745) !important;
                border-color: rgba(40,167,69,0.5) !important;
                box-shadow: 0 2px 16px rgba(40,167,69,0.4) !important;
            }}
            </style>
        """, unsafe_allow_html=True)
